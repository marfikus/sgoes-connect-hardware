// Данный скетч позволяет реализовать примитивную связь по протоколу Modbus RTU между смартфоном (или другим устройством), являющимся ведущим (master), и ведомым (slave), в данном случае датчиком-сигнализатором горючих газов СГОЭС. Дописать...
